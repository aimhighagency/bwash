	...
	attach: function (context, settings) {
		if(!jQuery('body').hasClass('mobile')){
			...
			jQuery(slide).each(function(index) {
			   ...
			   if(index == 0 && !jQuery.urlParam('index')){
			...
			function fadeItems(){
				...
				setTimeout(slidePos, trans_speed / 2);
			...
			function slidePos(){
				jQuery(slide).each(function(index) {
					...
					if($this.hasClass('art-tour')){
						...
						var actions = jQuery('.actions', this);
						var image_img = jQuery('.main-image img', this);
						var max_height;
						var max_width;
						var img_h = image_img.height();
						var img_w = image_img.width();

						image_img.css({'top' : '50%', 'margin-top' : -(img_h / 2)}); 
						caption.css({'margin-left' : -(img_w / 2),  'margin-bottom' : -(img_h / 2) - 40});
						actions.css({'margin-right' : -(img_w / 2),  'margin-bottom' : -(img_h / 2) - 40});
			...
			if(jQuery.urlParam('index')){
				dir = 'forward';
				current_item = jQuery.urlParam('index') - 1;
				setTourItem(jQuery.urlParam('index'));
			}
